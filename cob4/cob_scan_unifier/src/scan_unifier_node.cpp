...
void ScanUnifierNode::getParams()
{
    ...
    // Check if frame starts with '/' and delete it if so.
    if (frame_.at(0) == '/')
    {
        ROS_WARN("Frame parameter starts with '/'. This is not allowed. Deleting '/' from frame parameter.");
        frame_.erase(0, 1);
    }
}
    ...
    if (!current_scans.empty())
    {
        ...
        for (int j = 0; j < config_.number_input_scans; j++)
        {
            for (unsigned int i = 0; i < vec_cloud_[j].points.size(); i++)
            {
                ...
                if (index < 0 || index >= unified_scan.ranges.size())
                    continue;
